...
Your most recent FIT count you did was on `r last_count$date_from`, you recorded `r sum(last_count2$count, na.rm=T)` insects and the target flower was: `r last_count$target_flower`. `r if_else(sum(last_count2$count, na.rm=T)==0,"Even though you recorded no insects your FIT Count still provides us with valuable data about pollinating insects!","") `
...
if(sum(last_count2$count, na.rm=T)>0){
  ...
  labs(y = "Pollinator group",x = "Count")+
...
On average, you observed `r median(params$user_data$all_insects_total, na.rm=T)` insects per fit count. Let's take a look at the types of insects you most commonly observed per FIT Count, and how that compares to other FIT Counters:
